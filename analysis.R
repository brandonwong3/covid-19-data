...
national <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us.csv", stringsAsFactors = FALSE, header=TRUE)
view(national)

# Load the *state level* data into a variable. `states` 
states <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv", stringsAsFactors = FALSE, header=TRUE)
view(states)

# Load the *county level* data into a variable. `counties`
# (this is a large dataset, which may take ~30 seconds to load)
counties <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv", stringsAsFactors = FALSE, header=TRUE)
view(counties)
...
num_features_state <- ncol(states)
...
# `num_highest_state`
num_highest_state <- max(select(states, cases))
...
counties_in_state <- counties %>% 
  ...
  group_by(state) %>% 
  filter(deaths >= 0) %>% 
  summarize(nos_counties = n()) %>% 
  as.data.frame()
prop_no_deaths <- left_join(counties_in_state, zero_cases_county, by = "state") 
prop_no_deaths[is.na(prop_no_deaths)] = 0 
prop_no_deaths <- mutate(prop_no_deaths, prop = zero_deaths/nos_counties) %>% 
  select(state, prop)

# What proportion of counties in Washington have had zero deaths? 
# `wa_prop_no_deaths`
zero_cases_wa <- counties %>% 
  filter(counties == max(date)) %>% 
  group_by(state) %>% 
  filter(state == "Washington") %>% 
  filter(deaths == 0) %>% 
...
counties_in_wa <- counties %>% 
  ...
  summarize(nos_counties = n()) %>% 
  pull(nos_counties)
wa_prop_no_deaths <- zero_cases_wa/counties_in_wa
...
all_totals <- national %>% 
  select(date, cases) %>% 
  left_join(totals_by_day, national, by = "date")
view(all_totals)
...
num_state_diff <- all_totals %>% 
  mutate(state_nat_diff = (cases - state_total) != 0) %>% 
  pull(state_nat_diff) %>% 
  sum(na.rm = TRUE)
...
num_county_diff <- all_totals %>% 
  mutate(county_nat_diff = abs(cases - county_total) !=0) %>% 
  pull(county_nat_diff) %>% 
  sum(na.rm = TRUE)
...
joined_cases <- left_join(states, sum_county_to_state, by = c("date", "state"))
...
has_discrepancy <- joined_cases %>% 
  mutate(disparity = abs(cases - county_totals)) %>% 
  filter(disparity > 0) 
...
state_highest_difference <- joined_cases %>% 
  mutate(disparity = abs(cases - county_totals)) %>% 
  filter(disparity == max(disparity)) %>% 
  pull(state)
...
# Question 1
# What states have reported greater than 50,000 new cases as of the most recent
# date
states_50K_cases <- states %>% 
  filter(states == max(date)) %>% 
  filter(cases > 50000) %>% 
  select(state, cases)

#Question 2
# When did the the single day new cases first exceed 75000 nationally
first_70k_cases <- national %>% 
  filter(new_cases > 75000) %>% 
  filter(date == min(date)) %>% 
  pull(date)

# Question 3
# What county has the highest ratio of deaths to cases (deaths/cases) 
# as of the most recent date
highest_county_mortality <- counties %>% 
  filter(counties == max(date), cases > 0) %>% 
  mutate(county_mortality = deaths/cases) %>% 
  filter(county_mortality == max(county_mortality)) %>% 
  select(county, state, cases, deaths)
